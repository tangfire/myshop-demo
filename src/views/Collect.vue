    ...
    <div class="collect">
        ...
        <div class="content">
            <div class="goods-list" v-if="collectList.length > 0">
                <MyList :list = "collectList" :isDelete = "true" @delete-collect="handleDeleteCollect"></MyList>
            </div>
...
export default {
  ...
  activated () {
    // console.log('hello collect = ', this.$store.getters.getUser.user_id)
    // 获取收藏数据
    this.$axios
  ...
  methods: {
    handleDeleteCollect (productId) {
      this.collectList = this.collectList.filter(item => item.product_id !== productId)
    }
  }
}
</script>
